...
$$ \prod_{u\in V}(r_u-l_u+1) \sum_{e\in E} \sum_{x\in C}
\left(\sum_{u\in A_e\\[l_u,r_u]\ni x} \frac{1}{r_u-l_u+1}\right)
...
subtrees that I should insert before inserting them to avoid exceeding the memory limit. Wow, memory
management in competitive programming 🤮.
